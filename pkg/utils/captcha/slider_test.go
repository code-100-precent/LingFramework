...
func TestNewSliderCaptcha(t *testing.T) {
	captcha := NewSliderCaptcha(300, 150, 50, 5, 5*time.Minute, nil)
	if captcha == nil {
		t.Fatal("NewSliderCaptcha returned nil")
	}
	...
	if captcha.sliderSize != 50 {
		t.Fatalf("Expected sliderSize 50, got %d", captcha.sliderSize)
	}
}
...
func TestSliderCaptcha_Generate(t *testing.T) {
	...
	sliderImage, ok := result.Data["slider_image"].(string)
	if !ok || sliderImage == "" {
		t.Fatal("Result slider_image is empty")
	}
	if !strings.HasPrefix(sliderImage, "data:image/png;base64,") {
		t.Fatal("Result slider_image should be base64 encoded PNG")
	}
...
func TestSliderCaptcha_VerifyWithTolerance(t *testing.T) {
	captcha := NewSliderCaptcha(300, 150, 50, 5, 5*time.Minute, nil)
	result, err := captcha.Generate()
	if err != nil {
		t.Fatalf("Generate failed: %v", err)
