...
// MinioStore represents MinIO / S3 compatible storage
type MinioStore struct {
	...
	BaseURL   string `env:"MINIO_PUBLIC_BASE"` // Custom domain for public access (optional)
}
...
func (m *MinioStore) PublicURL(key string) string {
	...
	// Fallback to endpoint (note: direct access may require public read policy)
	scheme := "http://"
	if m.UseSSL {
		scheme = "https://"
	}
